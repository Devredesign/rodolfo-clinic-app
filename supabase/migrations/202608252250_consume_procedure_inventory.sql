create or replace function public.consume_procedure_inventory(
  ...
  p_procedure_id uuid,
  p_multiuse_depleted jsonb default '{}'::jsonb,
  ...
  for r in select pp.id,pp.product_id,pp.standard_quantity_snapshot,p.usage_type,p.name from public.procedure_products pp join public.products p on p.id=pp.product_id where pp.organization_id=p_organization_id and pp.procedure_id=p_procedure_id and pp.inventory_outcome is null order by pp.created_at,pp.id loop
    if r.usage_type='single_use' then
      ...
      for i in 1..v_qty loop
        select id into v_container from public.inventory_containers where organization_id=p_organization_id and product_id=r.product_id and status in ('open','closed') order by case when status='open' then 0 else 1 end,created_at limit 1 for update skip locked;
      ...
      select id,status into v_container,v_status from public.inventory_containers where organization_id=p_organization_id and product_id=r.product_id and status in ('open','closed') order by case when status='open' then 0 else 1 end,created_at limit 1 for update skip locked;
      if v_container is null then raise exception 'Not enough inventory for %',r.name; end if;
